...
不是所有字段都该由 agent（制图小助手）逐字手填。下表字段**由协作者可视化/编辑器产出**，agent 不做精确手填，只表达意图：

| 字段 | 生产方式 | 说明 |
|---|---|---|
| `id` | 编辑器自动连续编号 | agent 不指定具体编号 |
| `x` / `y` | **协作者在可视化编辑器**（`config_editors/map_editor_v01.01`）拖拽/点击画布生成 | agent **不手填精确坐标**。agent 产出的格子坐标只作初值/意图占位（如隐藏格 `900,900`），**正式排布以协作者在编辑器拖拽后的导出结果为准** |
| `destinations` | 编辑器连线生成 | agent 只说明"谁连到谁"的连通意图，最终连线由协作者在编辑器里拉出 |

**文本字段（`name` / `description` / `msg`）降低编辑门槛**：
- 协作者**不用逐字写**、也不用手写 JSON。只需一句话说清"这格叫什么、什么效果"（甚至只说效果），agent 据此**起草中文文案并自动补全 `{zh-CN,en-US}` i18n 外壳**，协作者确认/微调即可。
