...
export async function bootstrap(config: ServerConfig = loadConfig()): Promise<void> {
  ...
  // 创建应用
  const { httpServer, socketManager, economy, items, dayNightCycle, prosperityManager, eraManager } = createApp(config, {
  ...
  const shutdown = (signal: string) => {
    ...
    gracefulShutdown(httpServer, socketManager, economy, items, dayNightCycle, prosperityManager, eraManager)
      .then(() => {
        process.exit(0);
      })
      .catch((err) => {
