...
- **Payload**: `{ token: string }`（与 `ADMIN_TOKEN` 环境变量比对）
- **Ack**: `AckResult<{ sessionTimeout: number }>`（默认 24 小时）
...
- **Ack**: `AckResult`
- **副作用**：广播 `server.valueChanged`，记录审计日志。
...
- **Ack**: `AckResult`
- **副作用**：广播 `server.playerStatusChanged`。
...
- **Ack**: `AckResult`
- **副作用**：广播 `server.notification`、`server.playerLeft`，并从世界移除玩家。
