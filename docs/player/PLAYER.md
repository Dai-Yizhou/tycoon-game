...
组队当前提供邀请、接受、离队、踢出和队伍状态查询，聊天支持 global、team、region 频道。队伍更新时，`TeamHandler` 会通过 `GameWorld.updatePlayer()` 把共享数值写回成员：`money` 取全体成员平均值，`credit` 取全体成员最低值再加默认组队加成 5；地产、投资、税款和破产状态仍按玩家身份处理。大型地图需要额外关注多区域繁荣度、交通目的地、地产升级、投资持股、抵押、税收和破产清算之间的闭环。

大型地图的经济闭环为：起点资金与经过奖励进入现金流，地产购买和升级形成资产沉淀，合租与租金形成玩家间转移，投资形成长期收益入口，抵押提供流动性，税收按地图配置回收现金，现金耗尽后破产检查、复活或清算释放地产，繁荣度和交通网络再影响后续落点收益。当前服务端保留这些状态，但默认地图规模较小，不能据此推断大型地图已经完成平衡性验证。
