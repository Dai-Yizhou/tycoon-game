...
## 方向 12：有生产调用 ≠ 代码是活的（职责可能已被其他代码承担）

> 这是判断死代码的**第三层**，也是比"找调用方"更难的一层：
> - **第一层（前几节）**：没人调用 → 死。
> - **第二层**：只有测试调用、生产零调用 → 怀疑"纸面验收"（方向 9）。
> - **第三层（本节）**：**有生产调用，但该函数做的事已没有后果/已由别人做了** → 删掉运行行为不变，才是真死。别被"我 grep 到调用方了"骗过去。

### 12.1 一个铁证：`updatePerformanceMetrics()` 每帧被调用，但删了啥事没有

我实际核实 `BoardRenderer.ts`，发现有这么一段：
```ts
// BoardRenderer.render() 每帧末尾都会调它（L139）
this.updatePerformanceMetrics();

// L165-171
private updatePerformanceMetrics(): void {
  const now = performance.now();
  this.frameCount++;                        // 只写 frameCount
  if (now - this.lastFrameTime >= 1000) {
    this.frameCount = 0;
    this.lastFrameTime = now;               // 只写 lastFrameTime
  }
}
// L44-45 声明的两个私有字段：
...
**逐条核查**：
- 它有生产调用吗？**有**。`render()` 每帧（`requestAnimationFrame` 循环）都调 `updatePerformanceMetrics()`。
- 它做的事有后果吗？**没有**。`frameCount` 和 `lastFrameTime` 在**全项目没有任何一个读取点**——只有 L44/L45 的声明和 L167-171 的写入，从不被读、不被展示、不被用于判断。它原本想做的"FPS/性能监控"职责**已被移除或从未接入**（没有 console、没有 store、没有 stats 上报）。
- 删掉会怎样？**运行行为完全不变**。删了 `render()` 里的调用和这两个字段，画布照常渲染。

这就是"**有生产调用 ≠ 代码是活的**"的最清晰例证：调用是"真"的（每帧都在跑），但职责是"死"的（副作用为零）。

### 12.2 为什么会出现"有调用但职责已死"

常见成因：
1. **原本的副作用被剥离**：某功能原本要写 Store / 上报 / 画遮罩，后来重构去掉了消费端，但"生产方"调用没删干净（`updatePerformanceMetrics` 正属于这类——统计逻辑没了，计数保留）。
2. **结果被静默丢弃**：调用了返回值的函数，但调用处 `void x()` 或不用返回值（如 `BoardRenderer.hitTest:185` 的 `void this.camera.screenToWorld(...)`）。
3. **同一件事有两条实现，旧的还在被叫着**：新实现（如 `ClientRenderLoop.updateBoardTheme` 用 **DOM+CSS** 做昼夜）已经承担了旧实现（如 `DayNightRenderer` 想在 Canvas 上画遮罩）的职责，但旧类的调用/加载点没清，或注解仍声称"由它负责"。
4. **只写不读的状态**：函数在维护一批"只被自己写、从不被他人读"的字段，字段本身连带成了僵尸（`frameCount`/`lastFrameTime` 就是这样）。
...
1. **看它写了什么，再找"读它的地方"**：
   ```
   Grep: frameCount / lastFrameTime
   # 只有声明 + 自己写入 = 无消费者 → 写操作对系统无影响
   ```
   只要"被函数修改的每个变量/状态，都存在至少一个**其他读取点**"，这个函数才有意义；否则它是在维护无人读取的死状态。

2. **看它的副作用是否在真正的下游被消费**：
   - 若函数会 `store.setXxx(...)` → 找 `getXxx` 的消费点是否还活着；
   - 若函数会 `io.emit('server.xxx')` → 找客户端是否还有监听（方向 0.3）；
   - 若函数只改自己的私有字段 → 找这些私有字段有无 getter/外部读取。

3. **看该"职责"是否有另一份实现真正承担了**：
   - 昼夜：`updateBoardTheme`（DOM+CSS）在实际生效，`DayNightRenderer`（Canvas 遮罩）没接入 → 职责已转移。删 Canvas 版本不影响昼夜显示。
   - 相机：`panTo` 承担"跟随居中";`zoomBy/pan/startDrag` 想承担"缩放拖拽"但无 UI 入口 → 后者的职责根本没落地。
...
|---|---|---|
| 无任何调用 | 死 | 删 + 清桶 + 删相关测试 |
| 仅测试调用（纸面验收） | 死 | 删方法 + 改写/删测试（方向 9）|
| 有生产调用，但**副作用无消费者/职责已被他人承担** | **死（最容易误判为活）** | 删该调用与死状态，确认替换实现仍在（方向 12）|
| 有生产调用，且副作用被真实下游消费 | 活 | 保留 |

### 12.5 安全删除前的一条保底验证

"职责已转移"的删除，可以用**直接行为验证**兜底：把要删的调用/函数临时去掉，跑运行时与测试，看玩家可见行为是否变化。
- 对 `updatePerformanceMetrics`：删调用后 `pnpm build:client && pnpm test`，UI 无差异 → 确认。
- 对某"宣称负责 X"的旧类：删加载点后，手动跑对应交互（如昼夜切换）仍正常 → 确认职责在别处。
- 关键不是"没人报错"，而是"**行为不变**"。若删掉某函数后某功能失效，说明职责没转移，别删。

> **一句话**：先确认"它做的事有没有人读它做出来的结果"，再下"活/死"的结论。调用在 CPU 里跑，不等于它的结果有人在意。